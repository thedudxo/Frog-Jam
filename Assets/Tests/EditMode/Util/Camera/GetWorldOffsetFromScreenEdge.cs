...
namespace Tests.Utils
{
    public class GetWorldOffsetFromScreenEdge
    {
        ...
        {
            float output = screen.GetWorldPositionOffsetFromScreenLeftEdge(0.15f);
        ...
        public void OffsetNotConstrainedBetween01ThrowsException(
            float Offset)
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => screen.GetWorldPositionOffsetFromScreenLeftEdge(Offset));
        }
        ...
        float offsetPercent, float expectedOffset)
        {
            ...
            float output = screen.GetWorldPositionOffsetFromScreenLeftEdge(offsetPercent); //the method being tested
